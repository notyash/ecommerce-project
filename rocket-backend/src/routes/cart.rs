...
use crate::{AppState, dto::{auth::AuthenticatedUser, cart::{AllProductsInCart, ItemToAdd, ItemToRemove}}, errors::AppError, models::cart::CartMutationResponse,
...
#[post("/add", data="<item>")]
async fn add_product_to_cart(user: AuthenticatedUser, item: Json<ItemToAdd>, state: &State<AppState>) -> Result<Json<CartMutationResponse>, AppError> {
    let user_id = user.id;
    let cart = get_or_create_cart(user_id, state.inner()).await?;
    ...
    let payment_invalidated = invalidate_pending_checkout_if_exists(state.inner(), user_id, cart.id).await?;

    Ok(Json(CartMutationResponse { payment_invalidated: payment_invalidated }))

}

#[post("/remove", data="<item>")]
async fn remove_product_from_cart(user: AuthenticatedUser, item: Json<ItemToRemove>, state: &State<AppState>) -> Result<Json<CartMutationResponse>, AppError> {
    let user_id = user.id;
...
async fn decrement_product_in_cart(user: AuthenticatedUser, item: Json<ItemToRemove>, state: &State<AppState>) -> Result<Json<CartMutationResponse>, AppError> {
    let user_id = user.id;
    let cart = get_existing_cart(user_id, state.inner()).await?;
